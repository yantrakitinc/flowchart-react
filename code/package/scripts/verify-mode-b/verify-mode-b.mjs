#!/usr/bin/env node
// Mode B — HEAD-only stamp + freshness check.
//
// For every slice touched by the HEAD commit (vs HEAD~1):
//   (1) shape gate: status:locked, verified:100%, last_validated valid ISO-8601 UTC
//   (2) freshness gate: most recent commit touching the slice (excluding the lock
//       file itself) is ≤ last_validated + the SAME_COMMIT_GRACE_MS window
//
// Read-only. Refuses to validate any slice whose status is `unlocked`.
...
/**
 * Return absolute paths of every file changed in HEAD vs HEAD~1.
 * On a root commit (no HEAD~1) returns every tracked file in HEAD.
 *
...
export function main(io = {}) {
  ...
  if (locks.length === 0) {
    write("verify-mode-b: HEAD's changed paths don't resolve to any spec'd slice — nothing to stamp-check.\n");
    return { exitCode: 0, locksChecked: [], failures: [] };
  }

  write(`verify-mode-b: stamp-checking ${String(locks.length)} lock(s) for slices touched by HEAD\n`);
  ...
  if (failures.length === 0) {
    write(
      `verify-mode-b: OK (${String(locks.length)} lock(s) locked + verified=100% + fresh vs git history)\n`,
    );
    return { exitCode: 0, locksChecked: locks, failures: [] };
  }

  writeErr(`verify-mode-b: ${String(failures.length)} lock(s) failed\n`);
