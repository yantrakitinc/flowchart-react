# src/layout — standards compliance

`status: locked` — every scoped gate (source-coverage, docs, flow-coverage, no-undeclared-deps,
coverage-threshold, test-naming) passes for `src/layout/` as of `last_validated` below.
...
standards:
  ...
  PROCESS_DISCIPLINE: NOT REQUIRED
