# `src/react/nodes/` — standards compliance
...
for the same reason as the parent: this folder ships UI (`FlowNode.tsx`) and has not yet had its
real-browser walk (`browser_validated`) — no stamp is fabricated here either.
...
feature: code/package/src/react/nodes
