...
7. The slice's locked `__specs__/spec.yaml` + `spec.md` + `flows/*.flow.yaml` + `manual/*.yaml`
...
- **History goes in the Changelog story** (UI components) or git (everything else) — never in JSDoc/comments/spec.yaml.
...
- **spec.yaml is the implementation contract you build against; spec.md is the narrative.** If the spec needs narrative you find missing, that's a feature-spec-writer dialogue, not a spec.yaml addition.
...
### Reading `spec.yaml.scope_authority`
