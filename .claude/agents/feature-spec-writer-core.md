...
1. **Reproducibility audit (forward)** — caller hands you a locked `__specs__/spec.yaml` + the corresponding source code. You decide whether the spec captures the source's observable contract. Output: PASS / GAPS (numbered list of contract aspects in code but not in spec, OR in spec but not in code).
...
3. **Legacy backfill** — caller hands you existing source (no spec). You reverse-engineer `spec.yaml + spec.md + flows/*.flow.yaml + manual/*.yaml` from the source's actual behavior. Output: a complete `__specs__/` ready for human review.
...
- Stamp `__specs__/standards-compliance.yaml` (verifier owns the stamp).
- Commit. Push.
...
## Coverage of source
- Every export covered by a flow.yaml: YES | NO (list missing)
