# DIGEST for github-project-agent.md — GENERATED, do not edit. Regenerate: node ~/.claude/standards/scripts/generate/generate-agent-digests.mjs
# sources:
#   ISSUES.md c8da298087414c24
#   BRANCHES_AND_COMMITS.md ef0b0306386b739f
#   PULL_REQUESTS.md a73deaecd3122253
#   README_CONTRACT.md 9359e87d99d3dd41
...
- `standards_checklist`:
  ...
  - `required_groups`:
    - `spec_phase`: "spec.yaml + spec.md + flows/<fn>.flow.yaml + manual/<flow>.md (HTTP|UI) + ui/<flow>.md ui-locked (UI; USER sign-off) + openapi.yaml (HTTP) + asyncapi.yaml (events) + standards-compliance.yaml — SPEC_CONTRACT + FLOW_CONTRACT + MANUAL_FLOWS + WRITING_ORDER"
    ...
    - `verify`: "verifier Mode A green + standards-compliance.yaml stamped (status:locked, verified:100%) — WRITING_ORDER"
...
- `gate`: scripts/verify/verify-trunk-integration.mjs _(flags locked features on the branch that are absent from main)_
