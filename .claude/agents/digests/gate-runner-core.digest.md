...
# sources:
#   VERIFIER_MODES.md 6be99dfdbfa484ab
#   REPO_GATE_INSTALLATION.md 1f6febf91b189509
...
- `layers`:
  - `timestamp_ops`: letters A–F — read or write the `status:` / `last_validated:` fields on `__specs__/standards-compliance.md`
...
- `lock_state_machine`: 'every `__specs__/standards-compliance.md` carries `status: locked | unlocked`; Mode A flips to unlocked at start, runs gates, flips back to locked with fresh last_validated on pass; interrupt mid-gates → stays unlocked (signal: slice not re-validated); Modes B / B.5 / D / F / F-Random refuse to validate any unlocked slice'
- `implementation`: 'ONE dispatcher serves every mode: `node scripts/verify/verify-mode.mjs <letter-or-name>` where name ∈ {a, b, b5, c, d, e, f, inspect, pristine, cleanup-orphans, verify-all}. Mode `a` takes a slice path (`verify-mode.mjs a <slice>`). Read-only modes (b, b5, c, d, e, f, inspect, pristine, cleanup-orphans) NEVER write a lock; `a` and `verify-all` stamp; `inspect` always exits 0. Shared lock/freshness helpers (checkLockShape, checkFreshness, findAllLocks, stampLock, …) live in `scripts/lib/locks.mjs` — the single source consumed by the dispatcher AND the three standalone gates verify-standards-compliance --full (freshness pass) / verify-standards-compliance / verify-commit-stamp.'
- `timestamp_modes`:
