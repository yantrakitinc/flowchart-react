...
Source of truth for each mode's behavior: `~/.claude/standards/scripts/verify-mode-<mode>/verify-mode-<mode>.mjs`. The original A/B/C/D documentation below remains the canonical SOP for those modes; the 11 new modes follow the same shape (read locks → run gates → report verdict).
...
**Tools used inline (Mode A + B):** Bash, Glob, Grep, Read. The verifier never edits source / tests / specs even inline. The ONLY file the verifier writes is `__specs__/standards-compliance.yaml` on green Mode A.
...
11. On green: stamp `__specs__/standards-compliance.yaml` (`status: locked`, `verified: "100%"`, `last_validated: <utc-now>`)
...
**Steps:**
1. For every changed feature in the push, read its `__specs__/standards-compliance.yaml`
...
- Stamped `<absolute path to standards-compliance.yaml>` at <utc-iso-8601> | not-stamped (verdict ≠ SHIPPABLE)
...
- Required artifact missing (`spec.yaml` / `flow.yaml` / lock / blast-radius path)
