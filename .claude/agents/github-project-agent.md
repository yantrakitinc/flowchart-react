...
- **Description template** — every issue's body file follows this shape verbatim:
  ...
  ### Spec phase
  - [ ] __specs__/spec.md + flows/<fn>.flow.md + manual/<flow>.md (HTTP|UI) + ui/<flow>.md ui-locked + standards-compliance.md
  ### Code phase
  ...
  - [ ] verifier Mode A green + standards-compliance.md stamped; branch feat/NNNN-id; `<type>: <subject>`; no AI attribution; pushed
