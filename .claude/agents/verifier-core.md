...
- **Mode E** — entire-repo stamp update; you invoke Mode A against every slice that owns a `__specs__/standards-compliance.md`; you stamp on green. Used after sweeping changes (e.g., a new gate lands).
...
4. On green per slice: stamp `__specs__/standards-compliance.md` (`status: locked`, `verified: "100%"`, `last_validated: <utc-now>`).
...
- Required artifact missing (spec.md / flow.md / lock) — surface, BLOCKED for that slice/gate
