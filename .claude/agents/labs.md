...
- Every surface you create maps to an existing flow (`flows/<fn>.flow.md`) and/or journey. No flow behind it → do not build it. Never invent UI for an intent no flow models.
...
The walkthrough script is the flow's own `mermaid:` graph, extended so each node carries `{event, sequence, action, ui_binding}` (LABS#auto-walkthrough). Keep it single-origin with `flow.md` — steps map 1:1, never invent one. The player is in-app JS/React, a visual demo (not an E2E test), runs on any browser + mobile.
