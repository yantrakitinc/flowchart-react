...
Read this FIRST whenever the work touches `src/components/ui/**`, `src/features/<feature>/components/**`, `src/components/composites/**`, `src/app/**`, `packages/<pkg>/src/{sidepanel,popup,options}/**`, OR any file with `__specs__/spec.md` where `tier ∈ {atomic, composite, page}`.
...
| `page` | `src/app/<route>/`, `packages/<pkg>/src/{sidepanel,popup,options}/<name>/`, or any folder with `__specs__/spec.md` carrying `tier: page` | `~/.claude/standards/PAGE_CREATION.md` |
...
- `<tier>_spec_schema` — what spec.md must contain
...
Write the tier's spec.md, spec.md, design.md, flows/, manual/, standards-compliance.md. Use the tier-specific templates:

- Atomic: `~/.claude/standards/templates/{component-spec.md.template, component-design.md.template, ...}`
- Composite: `~/.claude/standards/templates/{composite-spec.md.template, composite-design.md.template, ...}`
- Page: `~/.claude/standards/templates/{page-spec.md.template, page-design.md.template, ...}`

Design.md MUST cite research.md.

Composite spec MUST list `consumed_atomics` with each entry's locked SHA pinned at lock time (the atomic's `git rev-parse HEAD` when the atomic's standards-compliance.md was last flipped to `status: locked`).
...
**spec.md is implementation-only; narrative goes in spec.md / design.md.** spec.md carries the impl contract (public_api, tokens_consumed, state_classes_consumed, test_coverage; optional interaction_states/touch_target/contrast). Anatomy / ARIA / keyboard / responsive / edge-cases → design.md. Concept / exploration / decisions → spec.md. Component HISTORY → the Changelog story (never JSDoc/comments/spec.md).

**Stories phase ships exactly 3 max:** Playground (required) + AllVariants (optional) + Changelog (required, newest-first). JSDoc is parent-facing only; inline comments minimal. `standards-compliance.md` = 4 fields only.
...
- **coder-core**: dispatch for Phase 2 + 3 of a single tier slice. Pass it the locked design.md path and the spec.md path. It writes tests + source.
