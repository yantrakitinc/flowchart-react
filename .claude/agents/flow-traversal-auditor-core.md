...
- Read ONLY `docs/flows/` — the human flow catalog (`00-INDEX.md` + the `<FF>-<NNN>-<name>.md` files) and the `flows/<fn>.flow.yaml` machine contracts they link to. Do NOT read code, journeys, requirements, or any other spec. If you cannot get from one point to the next using only the flow docs, that is the finding — do not go hunting in the code to rescue it.
