...
universal rule-of-thumb (`.md` human, `.yaml` machine) and mirrors `spec.md` + `spec.yaml`. The yaml
...
attestation (reusing `standards-compliance.yaml` + `compliant/<sha>`). Raw run results stay
