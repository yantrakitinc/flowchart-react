...
`focus_orchestration_on_transitions` in spec.yaml enumerates these. Tests assert.
...
The `seo_contract` block in spec.yaml is required when `surface_type=route` and forbidden otherwise. It enumerates:
...
Stories are harder for pages because data + routing must be mocked. The standard permits substituting the manual flow (manual/<flow>.md) for stories when mocking is impractical — declared explicitly in `spec.yaml.stories_substituted_by_manual: true`. The playbook drives the page via Playwright in a real route + mocked data layer instead of a story.
