...
- `discovery_model`:
  ...
  - `tier_markers`:
    ...
    `tier: page` marker in spec.md. Atomics need no tier marker; a composite under a
...
- `relationship_to_existing`:
  ...
  - `SPEC_CONTRACT.md`: __specs__/ folder + spec.md shape (features)
...
- `details`: 0_research:
    ...
    - `writes`:
      - __specs__/spec.md _(machine schema; component_spec_schema below)_
      ...
      - __specs__/flows/<interaction>.flow.md _(one per user-driven interaction)_
      - __specs__/manual/<flow>.md _(agent script; STARTS from Storybook URL, never app URL)_
      - __specs__/standards-compliance.md _(status:draft initially)_
    - `gate`:
      design.md carries an accepted lock marker (autonomy.design_lock_marker_accepted below;
      by:agent requires verify-component-design-self-lock-eligible to pass) AND spec.md
    ...
    - `forbidden_during_this_phase`:
      - editing __specs__/spec.md without re-locking design.md
    ...
    - `gate`: all green → flip __specs__/standards-compliance.md to status:locked + verified:100% + last_validated:<today>
...
- `specs`:
  ...
  - __specs__/spec.md _(required; IMPLEMENTATION-ONLY; validates against component_spec_schema)_
  ...
  - __specs__/flows/<interaction>.flow.md _(required; one per interaction; subagent walks these)_
  ...
  - __specs__/standards-compliance.md _(required; 4 fields only (status/verified/last_validated/feature))_
...
- `self_lock_eligibility`:
  ...
  - `blocks_when_any_true`:
    ...
    - spec.md contains 'tbd' / 'TBD' / 'TODO' / 'FIXME' / '<…>' placeholders
...
- `override_in_chat`: user may declare stakes per-component in chat; agent records it in the component's spec.md.stakes field

## component_spec_schema

- `doctrine`:
  spec.md is IMPLEMENTATION-ONLY. verify-component-spec.mjs requires EXACTLY the four
  required_blocks below — the contract a coder builds + a verifier checks against. Do NOT
  restate narrative in spec.md (see narrative_lives_elsewhere).
...
- `forbidden`:
  - prose rationale in spec.md (goes in spec.md)
  - design narrative in spec.md — anatomy/aria/keyboard/contrast/edge-cases (goes in design.md)
  - component history in spec.md (goes in the Changelog story)
...
- `verify-component-spec.mjs`:
  - `blocks_when`: __specs__/spec.md exists but missing any required_top_level_field OR required_block (per component_spec_schema)
...
- `step_4`:
  add pnpm scripts:
  "verify:component": "node ~/.claude/standards/scripts/verify/verify-component.mjs --check component-spec &&
    node ~/.claude/standards/scripts/verify/verify-component.mjs --check component-tokens &&
    node ~/.claude/standards/scripts/verify/verify-component.mjs --check component-state-vocab &&
    node ~/.claude/standards/scripts/verify/verify-component.mjs --check component-design-locked"
...
- writing spec.md after source ("spec mirrors code" is documentation, not a spec)
