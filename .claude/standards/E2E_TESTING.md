...
- `enforced_by`: scripts/verify/verify-e2e-db-safety.mjs _(pnpm verify chain; non-local conn strings + non-E2E_ DB env vars in e2e scope fail)_
...
- `form`: timestamped + <sha>-stamped (reuse standards-compliance.yaml + compliant/<sha>)
