...
- The spec is locked via `__specs__/standards-compliance.md` (`status: locked`).
...
- spec.md FIRST; flow.md next; code third.
...
- **Spec-driven, code-blind verification.** The spec is the single source of truth. The implementation, the E2E tests,
  ...
  spec.md + openapi/asyncapi + flows (NOT by reading the source), so executing it validates the running surface AND
...
- **Site blueprint before site code (SITE_BLUEPRINT).** A repo's page-tier surfaces are coded ONLY after a complete
  ...
  marker. Atomics/composites are exempt — design-system inventory builds anytime. Every page spec.md binds
...
└── __specs__/
    ...
    └── standards-compliance.md  # status:locked + verified:100% + last_validated
```
