...
A YAML file at `<feature>/__specs__/standards-compliance.yaml`. Required fields:
...
- **Presence**: every folder containing `__specs__/` has a `standards-compliance.yaml` with the required fields in the right state.
