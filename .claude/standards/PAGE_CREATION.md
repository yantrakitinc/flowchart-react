...
- `discovery`:
  ...
  `tier: page` marker in spec.md — NOT a hardcoded route path. Any spec-bearing folder
...
- `relationship_to_existing`:
  ...
  - `SITE_BLUEPRINT.md`:
    page-tier slices are gated on a locked site blueprint; every page spec.md carries
...
- `details`: 0_research:
    ...
    - `writes`:
      - __specs__/spec.md _(page_spec_schema below)_
      - __specs__/spec.md
      ...
      - __specs__/flows/<route-flow>.flow.md
    ...
    - `substitution_rule`:
      ...
      for stories; explicitly declare spec.md.stories_substituted_by_manual: true 5_verify:
    - `runs`:
    ...
    - `gate`: all green → flip standards-compliance.md to status:locked
...
- `verify-page-landmarks.mjs`:
  - `location`: ~/.claude/standards/scripts/verify/verify-component.mjs --check page-landmarks
  - `blocks_when`:
    - spec.md.landmark_structure declares a landmark that has no matching element_tag in source
    - source uses a landmark element NOT declared in spec.md.landmark_structure (e.g. unexpected <nav>)
    - skip-link declared in spec.md but target element does not exist in source
  - `exit_code`: 0 on green, 1 on red
- `verify-page-state-machine.mjs`:
  - `location`: ~/.claude/standards/scripts/verify/verify-component.mjs --check page-state-machine
  - `blocks_when`:
    - >- spec.md.state_machine declares a state with no corresponding render branch in source (grep heuristic: the state name as a discriminator in render conditionals)
    - spec.md.state_machine declares a transition with no test in __tests__
