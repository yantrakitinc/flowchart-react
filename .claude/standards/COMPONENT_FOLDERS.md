...
- `shape`:
  ...
  └── __specs__/                       # AI-facing landing zone
      ├── spec.md                    # machine-readable IMPLEMENTATION contract ONLY
      ├── spec.md                      # human prose: concept + exploration + decisions (the narrative)
      ├── flows/<fn>.flow.md         # per exported function
      ...
      └── standards-compliance.md    # status + verified + last_validated + feature (4 fields, nothing more)
- `rules`:
...
- `rule`:
  Every primitive's spec.md MUST declare exactly the `state-*` classes and `var(--*)` tokens
...
- `enforced_by`: ~/.claude/standards/scripts/verify/verify-component.mjs --check component-spec-vs-source
- `scope`:
