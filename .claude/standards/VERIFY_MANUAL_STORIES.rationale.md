...
Manual verification of a component is only trustworthy when the instructions are derived from the CONTRACT, not from the code. A Verify-Manual story authored code-blind from spec.yaml + design.md + the manual flow script tests the component against what it promises — and simultaneously proves the spec is complete enough to drive a browser agent. A flow that cannot be written from the spec exposes a deficient spec, which is the defect to fix (`MANUAL_FLOWS.md`).
