...
1. **Brainstorm** → the spec is born. Lands in `<feature>/__specs__/spec.yaml + spec.md`. NOT a separate timestamped doc.
...
7. **Verify** → run `verify-standards-compliance` (presence + freshness gate) + the manual walk + stamp `__specs__/standards-compliance.yaml`.
8. **Finish branch** → merge / PR / cleanup options.

This integrates the plugin's skill outputs with the standards' artifacts. Brainstorming doesn't dump into `docs/superpowers/specs/` — it writes the actual per-feature spec.yaml. Verification doesn't produce a `code-confidence.md` — it stamps the standards-compliance.yaml lock file.
...
- **Specs** land in `<feature>/__specs__/spec.yaml + spec.md`. The brainstorming output IS the per-feature spec; there's no separate "design doc" that gets translated later.
