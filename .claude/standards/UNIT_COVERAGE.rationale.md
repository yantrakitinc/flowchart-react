...
Required scenarios per function come from `flow.md`:
- happy path
...
The durable record of done is the per-feature `__specs__/standards-compliance.md` lock file (see LOCK_FILES.md). The script's output is the moment-in-time proof; the lock file is the persistent state.
