...
- `discovery`:
  ...
  marker in spec.yaml (so a composite may live anywhere). The marker is the true gate — an
...
- `relationship_to_existing`:
  - `COMPONENT_CREATION.md`:
    ...
    live in the atomic's own spec.yaml + design.md).
...
- `details`: 0_research:
    ...
    - `writes`:
      - __specs__/spec.yaml _(composite_spec_schema below)_
      ...
      - __specs__/flows/<interaction>.flow.yaml
      - __specs__/manual/<flow>.md
      - __specs__/standards-compliance.yaml
    - `gate`:
      design.md carries an accepted lock marker; consumed_atomics in spec.yaml all resolve to
      existing atomic folders AND those atomics are status:locked AND each frozen_prop is
      valid against the atomic's spec.yaml.public_api.props schema
    ...
    - `gate`: all green → flip standards-compliance.yaml to status:locked + verified:100% + last_validated:<today>
...
- `required_blocks`:
  ...
  - `consumed_atomics`:
    ...
    - `rule`:
      every entry resolves to an atomic folder containing __specs__/standards-compliance.yaml
  ...
  - `frozen_props`:
    ...
    - `rule`: every frozen value is a literal AND matches the type declared in the atomic's spec.yaml.public_api.props[prop_name].type
...
- `verify-composite-composition.mjs`:
  - `location`: ~/.claude/standards/scripts/verify/verify-composite-composition.mjs
  - `blocks_when`:
    ...
    - any frozen_prop type doesn't match the atomic's spec.yaml.public_api.props[prop].type
...
- `verify-composite-state-propagation.mjs`:
  - `location`: ~/.claude/standards/scripts/verify/verify-composite-state-propagation.mjs
  - `blocks_when`:
    - >- spec.yaml.state_propagation declares a rule with no corresponding implementation in source (grep-based: look for the prop_set_on_atomic name in the composite's source under the atomic_target's render context)
