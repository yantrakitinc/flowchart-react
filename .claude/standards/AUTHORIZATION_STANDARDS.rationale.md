...
1. **Consistency with the rest of the regiment.** Every other contract in `__specs__/` is declarative YAML — spec.yaml, openapi.yaml, asyncapi.yaml, standards-compliance.yaml (manual flows are the one markdown contract). Permission rules in raw SQL would be the lone exception.
