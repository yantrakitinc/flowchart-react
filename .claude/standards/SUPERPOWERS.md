...
## workflow 1: user describes idea → /brainstorm → design spec (lands in the relevant <feature>/__specs__/spec.yaml +
...
`verify-standards-compliance` + manual walk; stamp __specs__/standards-compliance.yaml) 8: all done →
...
- `specs`:
  - `location`: <feature>/__specs__/spec.yaml + spec.md
