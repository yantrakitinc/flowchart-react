...
The atomic's `__specs__/standards-compliance.md` at `status: locked` is the contract the composite trusts. Re-documenting the atomic inside the composite breaks that trust model in two ways:
...
These propagation rules are explicit in spec.md's `state_propagation` block. The verifier (`verify-composite-state-propagation`) grep-checks that the declared rule is implemented:
...
The composition_edge_cases section in spec.md + design.md focuses on interactions BETWEEN atomics. Examples:
