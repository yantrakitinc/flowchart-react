...
## Why a fenced machine block inside `flow.md`

The flow doc's content is 90% structured (16 required keys, named paths, ai_agent_action sub-map) and 10% prose. The structured part lives in a single fenced ```yaml block — one format every reader (verify script, agent, generator) parses via `scripts/lib/load-spec.mjs`, which extracts that block. Wrapping it in a Markdown doc lets the file also carry agent/human-readable prose around the block without a second parser mode: the loader reads only the fenced block, so automated checks stay simple while the doc stays readable. The single prose-y field is `mermaid:` (the diagram), a multiline YAML string inside the block. The `paths:` section is a structured map (`happy`, `error_*`, `edge_*`), not free-form prose.

## Each of the 17 required `flow_md` keys (+ 1 optional) — why each one
...
- `spec` — path to the spec.md. The contract anchor.
