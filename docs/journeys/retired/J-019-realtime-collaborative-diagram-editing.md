...
status: retired
retired_reason: >-
  no network/websocket/CRDT surface exists anywhere in the package (a pure client-rendering library with zero DB/HTTP of its own, per every feature spec's cross_cutting.auth: 'n/a') — there is no shared mutable state to synchronize.
---
