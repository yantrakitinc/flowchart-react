...
status: retired
retired_reason: >-
  the package renders read-only diagrams (playback + path selection only) and keeps no edit history to undo/redo — src/react/__specs__/spec.md: 'Persisting selection or playback state across sessions/reloads ... both are in-memory only', and there is no diagram-mutation surface at all.
---
