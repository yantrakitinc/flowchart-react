...
status: retired
retired_reason: >-
  no image-export capability exists or is planned — the package's only dependencies are @xyflow/react and dagre (package.json), neither of which does canvas/DOM-to-image work, and no such export surface appears in any feature spec.
---
